...
public class HexViewStash implements Observer {
    ...
    public HexViewStash(Level level, JPanel contentPanel) {
        if (level == null) {
            throw new NullPointerException(
                    "Use the other constructor if you don't actually have a level.");
        }
    ...
    /**
     * Validates that there are the same number of hexominos as there are
     * HexominoBullpenViews, and that there are no duplicate items in either
     * list. Throws an IllegalStateException if that's not true.
     */
    private void validateHexominoListState() {
        // Valide sizes are the same.
        int hexViewSize = this.getHexominos().size();
        int bullpenSize = this.bullpen.getPieces().size();
        if (hexViewSize != bullpenSize) {
            throw new IllegalStateException(
                    "Different number of hexominos in bullpen and bullpenview.");
        }

        // Validate no duplicate views.
        for (HexominoBullpenView v : this.getHexominos()) {
            int freq = Collections.frequency(this.getHexominos(), v);
            if (freq > 1) {
                throw new IllegalStateException(
                        "There are duplicate Hexominos in the bullpen");
            }
        }
    }
    ...
    public void refresh() {
        ...
        if (this.hexominosList == null) {
            throw new NullPointerException(
                    "Hexomino List was null when refreshing the view");
        }
        clearHexominoViews();
