...
public class BoardView extends JPanel implements Observer {
    ...
    public void setShadow(List<Point> pointsToShadow) {
        for (TileView tv : this.tileViews) {
            if (tv.setShadow(
                    pointsToShadow.contains(new Point(tv.row, tv.column))))
                tv.updated();
        }
    }
    ...
    public void updateHints() {
        ...
        for (Hint h : hints) {
            absoluteHintLocations.addAll(this.board.getPoints(h.getAnchor(),
                    h.getHexominoNumberSet()));
        }

        for (TileView tv : this.tileViews) {
            if (absoluteHintLocations
                    .contains(new Point(tv.getRow(), tv.getColumn()))) {
                tv.setHint(true);
            } else {
                tv.setHint(false);
            }
